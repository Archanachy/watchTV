...
const getParticularContent = async (contentId) => {
  ...
  SELECT 
    ...
    COALESCE(COUNT(DISTINCT r.user_id), 0) AS total_ratings -- Count distinct user_id to prevent duplicate counts
  FROM content c
  JOIN users_registration u ON c.id = u.id  -- Correct the join condition if necessary
